...
114. 🔧 THE ASSET LIBRARY (owner "lead with generation", 2026-07-05 — Opus 4.8 executing): a MASSIVE cohesive, license-clean, pixel-verified, style-locked top-down gothic library to end zone sparsity + retire the zone_builder ColorRect composites. Built $0 on the 5070 Ti. `tools/assets/`: **generate.py** (ComfyUI SDXL+Pixel-Art-XL inanimate lane + Wan 2.2 TI2V img2vid animated-creature lane), **interpret.py** (#112 interpreter/cleaner/SPOTLESS verifier/splitter/montage/library index), **asset_specs.py** (spec catalog: harbor/props/monuments/buildings/nature/fishing + the 5 ColorRect targets pier/cargo/crane/salt_pan/ledger_tablet), **scout.py** (secondary free-pack license scout). Cleaning bar (owner "spotless"): bg-removed + binary-alpha (no halo) + one-sprite-per-cell (connected-component split) + quantized + gothic-palette + alpha-trimmed; montage = the eyeball gate. Video-gen choice = **Wan 2.2 TI2V-5B** (already installed, fits 16 GB, proven-consistent img2vid — same flow as commercial sorceress.games, replicated free; see LEARNED_PRINCIPLES.md). Master index `_downloads/_assetlib/library.json`; catalog `design/ASSET_LIBRARY.md` (real counts + wiring plan). ASSETS ORIGINAL (generated, CC0/owner-owned) — never copied. Wiring into zones = Fable (visual law).
